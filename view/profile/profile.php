...
echo '<div class="col-md-10 repeatedDiv-profile search_result">
	...
	<div class="col-md-12">
		...
		<div class="action right">
			<button class="btn btn-xs btn-info edit-post-btn_'.$postId.'" data-toggle="modal" data-target="#editPost" data-editUserId='.$userId.' data-editPostId='.$postId.' onclick="editPost('.$postId.')"><i class="fa fa-pencil"></i> Edit</button>
			<button class="btn btn-xs btn-danger delete-post-btn_'.$postId.'" data-toggle="modal" data-target="#deletePost" data-delPostId='.$postId.' onclick="deletePost('.$postId.')"><i class="fa fa-trash"></i> Delete</button>
		</div>
		</div>
		...
		<div class="col-md-5 profile-div-right">';
		  echo '<input type="hidden" name="userId" id="userId_'.$postId.'" value="'.$userId.'">
				...
				/*echo '<span id="yes_cnt" class="yesCount_'.$postId.'">'.$cntYes.' </span>&nbsp;&nbsp;';
				 if($countofYes == 0){
				echo '<label data-toggle="tooltip" title="Trustable" onclick="postYes('.$postId.');"><i class="fa fa-heart-o" style="cursor: pointer;"></i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label>'; } if($countofYes == 1){  echo '<a href="#"  data-toggle="tooltip" title="Trustable" ><i class="fa fa-heart like-heart" id="activeIcon_'.$postId.'"></i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>';  } else{ echo'<i class="" id="inActiveIcon_'.$postId.'"></i>';}
...
<script>
	function deletePost(delete_ids){
		var post_id = $(".delete-post-btn_"+delete_ids).attr("data-delPostId");
		$("#delete_post").val(post_id);
	}

	function editPost(edit_ids){
		var user_id = $(".edit-post-btn_"+edit_ids).attr("data-editUserId");
		var post_id = $(".edit-post-btn_"+edit_ids).attr("data-editPostId");
		$.ajax({
			url: '../../controller/editPostDetailsController.php',
			type: 'post',
			data: { edit_post_id: post_id },
			success:function(response){
			    $('.edit-post-modal').html(response);
			}
		});
	}
</script>
