<?php
	...
	$sql = "SELECT R.`register_id` AS registerId, R.`first_name` AS firstName, R.`last_name` AS lastName, R.`user_image` AS userImage, P.`post_id` AS postId, P.`post_first_name` AS postFname, P.`post_last_name` AS postLname, P.`title_discussion` AS postTitle, P.`job_location` AS jobLocation, P.`job_role` AS jobRole, P.`linked_url` AS linkedUrl, P.`details` AS postDetails,P.`ratings` AS rating, P.`date_back_out` AS backOutDate, P.`resume` AS resume, P.`created_at` AS postCreated FROM `register_tbl` AS R JOIN `post_details_tbl` AS P ON R.register_id = P.`created_by` WHERE R.`status` = '1' ORDER BY P.`created_at` DESC";
	...
	<div class="col-md-10">
		<h5 class="home-headline">'.ucfirst($fname).' '.ucfirst($lname).'
			...
			echo '<a href="#" class="btn btn-primary btn-xs">Request Sent</a>';
			 }else{ echo '<a href="#" data-userid="'.$userId.'" data-registerid="'.$registerId.'" class="btn btn-primary btn-xs follow_btn"> <i class="fa fa-plus"></i> Connect </a>'; } 
		echo '</h5>
		...
		echo '<table width="100%" style="color: #0045ab;">
			<tr>
				...
				<td width="20%">'; if ($linkedUrl != ''){ echo '<a href="'.$linkedUrl.'" target="_blank" class="btn btn-sm btn-info"> <i class="fa fa-linkedin"></i></a>'; }  if ($resume != ''){ echo '<a href="../../resumeFiles/'.$resume.'" target="_blank" class="btn btn-sm btn-danger"><i class="fa fa-download"></i></a>'; } echo '</td>
			</tr>
		</table><br>
	...
	<div class="col-md-12">
		...
		<div class="col-md-6 social-icons-home">
			...
			<input type="hidden" name="register_id" id="register_id_'.$i.'" value="'.$registerId.'">';
				...
				<a class="collapsible" type="button" title="Comments" onclick="getCommentDiv('.$i.');"><i class="fa fa-comment" style="color: #337ab7"></i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
